...
SELECT cupcakes.name, SUM(orders.num_cupcakes) FROM cupcakes LEFT JOIN orders on cupcakes.id=orders.cupcake_id GROUP BY cupcakes.name ORDER BY cupcakes.name ASC;
...
SELECT email FROM customers LEFT JOIN orders ON customers.id=orders.customer_id GROUP BY customers.email HAVING SUM(orders.num_cupcakes) > 100 ORDER BY email DESC;
...
SELECT fname, lname, email FROM customers LEFT JOIN orders on customers.id=orders.customer_id LEFT JOIN cupcakes on orders.cupcake_id=cupcakes.id WHERE cupcakes.name='funfetti'AND orders.processed = TRUE GROUP BY customers.fname,customers.lname,customers.email;